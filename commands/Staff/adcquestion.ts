...
export = {
    ...
    async execute(msg: Message | ChatInputCommandInteraction) {
        ...
        if (msg.type === InteractionType.ApplicationCommand) {
            opt = (msg.options.getString("jogo")?.toLowerCase()) ? msg.options.getString("jogo")?.toLowerCase() : "0"
            informativo = (msg.options.getString("informativo")?.toLowerCase()) ? msg.options.getString("informativo")?.toLowerCase() : ""
        } else {
        ...
        switch (opt) {
            ...
            case "0":
                try {
                    ...
                    msg.reply({content: "Pergunta Adicionada"})
                } catch (e) {
                    logger.error(e)
                }
                break
